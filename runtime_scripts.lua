...
local function mkdir(path)
    if core.mkdir then return core.mkdir(path) end
    if minetest and minetest.mkdir then return minetest.mkdir(path) end
    return false
end
...
local function normalize_relpath(path)
    ...
    for part in path:gmatch("[^/]+") do
        part = tostring(part or ""):gsub("[^%w_%.%-]", "_"):gsub("%.%.", "__")
...
function M.save_script(player_name, filename, code, meta)
    ...
    if parent then ensure_dir_tree(parent) end
