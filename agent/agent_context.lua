...
function prompt_builder.build_system_prompt(player_name, options, state, deps)
    ...
    local lines = {
        ...
        "- Canonical skill invocation: local res = llm_connect.skills.<skill_id>.run(\"<tool_name>\", { ... }, player_name).",
        "- Never call skill tools directly as functions or nested methods. Wrong: llm_connect.skills.<skill_id>.<tool_name>(...) or llm_connect.skills.<skill_id>.<tool_name>:run(...).",
        "- Always check skill results before done=true: if not (res and res.ok) then return {done=false, continue=true, message=res and (res.message or res.error) or 'Skill failed'} end.",
        "- Correct skill completion pattern: local res = llm_connect.skills.<skill_id>.run(\"<tool_name>\", args, player_name); if not (res and res.ok) then return {done=false, continue=true, message=res and res.message or 'Skill failed'} end; return {done=true, message=res.message or 'Done'}.",
