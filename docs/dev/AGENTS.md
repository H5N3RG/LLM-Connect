# AGENTS.md - Entry Guide for Future Codex Sessions

This file is the short entry point. Detailed maintenance knowledge now lives in
the fixed-schema subsystem docs:

- `docs/dev/ROADMAP.md`: release gates and medium-term direction.
- `docs/dev/subsystems/agent.md`: dual-channel loop, retries, continuation.
- `docs/dev/subsystems/context.md`: context registry/facade/cache contract.
- `docs/dev/subsystems/runtime.md`: sandbox, policy, safe-core boundaries.
- `docs/dev/subsystems/skills.md`: skill registry and skill API contracts.
- `docs/dev/subsystems/gui-settings.md`: config GUI and setting handling.
- `docs/dev/subsystems/testing-release.md`: trace-driven verification workflow.
- `docs/dev/TESTS.md`: manual in-game regression scenarios.
- `ISSUES.md`: current log-derived bugs and TODOs.

## How To Start A Session

1. Read the bottom of `debug.txt`, `llm_user_prompt_log.txt`, and
   `llm_response_prompt_log.txt` when investigating agent behavior.
2. Read `ISSUES.md` for current live findings.
3. Read the relevant subsystem doc before changing code.
4. Use `ROADMAP.md` to decide whether a concern blocks `1.2.0` or belongs in a
   later contract pass.
5. Verify changed Lua files with `luajit -b`; use `lemr` only as a coarse load
   smoke test.

## Current Architectural Posture

LLM-Connect should now be treated as a small Lua-first runtime framework, not a
single-purpose chat mod. Keep changes small, traceable, and contract-driven.

Current 1.2.0 focus:

- Stabilize the agent loop, context cache, skill exposure, and config GUI.
- Keep root override settings explicit and opt-in.
- Prefer documented Lua-first skill functions over compatibility aliases.
- Avoid reintroducing JSON-heavy tool dispatch.

Important current contracts:

- Agent actions use fenced `lua_action` blocks.
...
The older notes below are retained as implementation history. Treat them as
secondary to the subsystem docs and current logs; some listed issues may already
be fixed.
